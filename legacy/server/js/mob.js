...
module.exports = Mob = Character.extend({
  ...
  hates: function (playerId) {
    return _.any(this.hatelist, (obj) => obj.id === playerId);
  },

  increaseHateFor: function (playerId, points) {
    if (this.hates(playerId)) {
      _.detect(this.hatelist, (obj) => obj.id === playerId).hate += points;
    } else {
  ...
  getHatedPlayerId: function (hateRank) {
    var i,
      playerId,
      sorted = _.sortBy(this.hatelist, (obj) => obj.hate),
  ...
  forgetPlayer: function (playerId, duration) {
    this.hatelist = _.reject(this.hatelist, (obj) => obj.id === playerId);
