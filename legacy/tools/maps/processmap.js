...
module.exports = function processMap(json, options) {
  ...
  // Object layers
  _.each(Tiled.objectgroup, (objectlayer) => {
    if (objectlayer.name === 'roaming' && mode === 'server') {
      ...
      _.each(objectlayer.object, (area) => {
        var chestArea = {
        ...
        _.each(area.properties.property, (prop) => {
          if (prop.name === 'items') {
            chestArea['i'] = _.map(prop.value.split(','), (name) => Types.getKindFromString(name));
          } else {
      ...
      var count = 0;
      _.each(objectlayer.object, (checkpoint) => {
        var cp = {
          id: ++count,
