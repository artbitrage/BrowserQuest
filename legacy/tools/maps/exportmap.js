...
function main() {
  getTiledJSONmap(source, (json) => {
    ...
    if (mode === 'client') {
      ...
      fs.writeFile(destination + '.json', jsonMap, (err, file) => {
      ...
      fs.writeFile(destination + '.js', jsonMap, (err, file) => {
        log.info('Finished processing map file: ' + destination + '.js was saved.');
      });
    } else {
      fs.writeFile(destination, jsonMap, (err, file) => {
