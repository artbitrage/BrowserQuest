  ...
  var Renderer = Class.extend({
    ...
    drawTerrain: function () {
      ...
      this.game.forEachVisibleTile((id, index) => {
        if (!m.isHighTile(id) && !m.isAnimatedTile(id)) {
          ...
          this.drawTile(this.background, id, this.tileset, tilesetwidth, m.width, index);
        }
      }, 1);
    },
    ...
    drawAnimatedTiles: function (dirtyOnly) {
      ...
      this.game.forEachAnimatedTile((tile) => {
        if (dirtyOnly) {
          if (tile.isDirty) {
            this.drawTile(this.context, tile.id, this.tileset, tilesetwidth, m.width, tile.index);
            tile.isDirty = false;
          }
        } else {
          this.drawTile(this.context, tile.id, this.tileset, tilesetwidth, m.width, tile.index);
    ...
    drawHighTiles: function (ctx) {
      ...
      this.game.forEachVisibleTile((id, index) => {
        if (m.isHighTile(id)) {
          this.drawTile(ctx, id, this.tileset, tilesetwidth, m.width, index);
