...
import { ChestArea } from './entities/areas/ChestArea.js';
import { Item } from './entities/Item.js';
import { Map } from './Map.js';
import * as Messages from '../protocol/Messages.js';
import { MobArea } from './entities/areas/MobArea.js';
...
export class WorldServer {
  ...
  constructor(id: string, maxPlayers: number, websocketServer: any) {
    ...
    this.onPlayerConnect((player) => {
      player.onRequestPosition(() => {
        ...
        // If player already has a position (loaded from DB), return it
  ...
  async run(mapFilePath: string) {
    ...
    this.interval = setInterval(() => {
      ...
      if (updateCount < regenCount) {
        ...
        this.regen_callback?.();
        this.syncPlayers(); // Sync players to DB on regen tick
        updateCount = 0;
      }
    }, 1000 / this.ups);
  ...
  async syncPlayers() {
    ...
    log.debug(`Syncing ${players.length} players to database...`);
    await Promise.allSettled(players.map((p) => p.save()));
  }
